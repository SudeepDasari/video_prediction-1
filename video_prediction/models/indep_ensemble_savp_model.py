...
from video_prediction.models import EnsembleSAVPVideoPredictionModel
from video_prediction.utils import tf_utils
from . import vgg_network
import pdb


class IndepEnsembleSAVPVideoPredictionModel(EnsembleSAVPVideoPredictionModel):
    def get_default_hparams_dict(self):
        default_hparams = super(IndepEnsembleSAVPVideoPredictionModel, self).get_default_hparams_dict()
        hparams = dict(
            num_views=1,
        )
        ...
        for i in range(self.hparams.num_views):
            ...
            with tf.variable_scope('view%d' % i):
                outputs_view, losses_view, metrics_view = \
                    EnsembleSAVPVideoPredictionModel.tower_fn(self, inputs_view, targets=targets_view)
            if i == 0:
